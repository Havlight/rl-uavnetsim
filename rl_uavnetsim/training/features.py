...
from rl_uavnetsim.rl_interface.mdp import _normalize_uav_position, _normalize_uav_velocity, _normalize_user_position, _safe_norm
...
def build_compact_local_observation(
    ...
    *,
    ...
    self_features = np.array(
        [
            *_normalize_uav_position(uav.position).tolist(),
...
def build_compact_state(
    ...
    for uav in ordered_uavs:
        uav_geometry.extend(_normalize_uav_position(uav.position).tolist())
    ...
    for user in sorted(users, key=lambda item: item.id):
        user_positions.extend(_normalize_user_position(user.position).tolist())
